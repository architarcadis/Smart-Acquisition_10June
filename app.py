...
def initialize_session_state():
    ...
    session_vars = {
        ...
        'df_procurement_routes': pd.DataFrame(),
        'df_team_capacity': pd.DataFrame(),
        'df_market_concentration': pd.DataFrame(),
        'df_t1_supplier_health': pd.DataFrame(),
        'df_supply_chain_kpis': pd.DataFrame(),
        'market_scan_config': {},
        'pinned_insights': [],
    ...
    if not st.session_state.sample_data_loaded:
        if st.sidebar.button("📥 Load Sample Data"):
            ...
            with st.sidebar.status("Loading Thames Water AMP 8 data...", expanded=True) as status:
                ...
                from utils.data_generator import (generate_market_segments_data, generate_demand_pipeline_data, 
                                                generate_sub_tier_map_data, generate_procurement_routes_data,
                                                generate_team_capacity_data, generate_market_concentration_data,
                                                generate_t1_supplier_health_data, generate_supply_chain_kpi_data)
        ...
        if st.sidebar.button("🗑️ Clear All Data"):
            ...
            for key in ['df_market_segments', 'df_competencies', 'df_demand_pipeline', 
                       ...
                       'df_sub_tier_map', 'df_supply_chain_risks', 'df_procurement_routes',
                       'df_team_capacity', 'df_market_concentration', 'df_t1_supplier_health',
                       'df_supply_chain_kpis']:
